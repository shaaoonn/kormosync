...
const __dirname$1 = require$$1$1.dirname(node_url.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("main.js", document.baseURI).href));
require$$1$3.app.commandLine.appendSwitch("disable-extensions");
require$$1$3.app.commandLine.appendSwitch("disable-translate");
require$$1$3.app.commandLine.appendSwitch("disable-sync");
require$$1$3.app.commandLine.appendSwitch("disable-features", "TranslateUI");
process.on("uncaughtException", (error2) => {
  console.error("Uncaught Exception (Main Process):", error2.message);
});
process.on("unhandledRejection", (reason) => {
  console.error("Unhandled Rejection (Main Process):", reason);
});
const DIST = require$$1$1.join(__dirname$1, "../dist");
const VITE_PUBLIC = require$$1$3.app.isPackaged ? DIST : require$$1$1.join(DIST, "../public");
let mainWin = null;
let tray = null;
...
function createMainWindow() {
  mainWin = new require$$1$3.BrowserWindow({
    icon: require$$1$1.join(VITE_PUBLIC, "electron-vite.svg"),
    webPreferences: {
      preload: require$$1$1.join(__dirname$1, "preload.js"),
      backgroundThrottling: false,
      // Critical: Prevent timer from pausing in background
      spellcheck: false,
      // Save memory — no dictionary loaded
      enableWebSQL: false,
      // Deprecated, save memory
      v8CacheOptions: "bypassHeatCheck"
      // Cache V8 bytecode aggressively — less re-compilation
    },
    width: 1100,
    height: 750,
    minWidth: 800,
    ...
    show: false
    // Don't show until ready — prevents blank white flash and reduces initial memory spike
  });
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    mainWin.loadFile(require$$1$1.join(DIST, "index.html"));
  }
...
function createTray() {
  ...
  const icon = require$$1$3.nativeImage.createFromBuffer(canvas, { width: size, height: size });
...
function getDeviceId() {
  ...
  try {
    ...
    if (process.platform === "win32") {
      try {
        const machineGuid = require$$1$4.execSync(
    ...
    const hash = require$$0$3.createHash("sha256").update(components.join("|")).digest("hex");
    cachedDeviceId = hash.substring(0, 16);
...
require$$1$3.ipcMain.handle("GET_DEVICE_ID", async () => {
  return getDeviceId();
});
require$$1$3.ipcMain.handle("CAPTURE_SCREENSHOT", async () => {
  try {
    const sources = await require$$1$3.desktopCapturer.getSources({
      types: ["screen"],
    ...
    console.error("Failed to capture screenshot:", error2);
    throw error2;
  }
});
require$$1$3.ipcMain.handle("GET_ACTIVITY_STATS", async () => {
  return { keystrokes, mouseClicks };
});
require$$1$3.ipcMain.handle("RESET_ACTIVITY_STATS", async () => {
  keystrokes = 0;
  mouseClicks = 0;
  return { success: true };
});
require$$1$3.ipcMain.handle("OPEN_EXTERNAL", async (_, url) => {
  await require$$1$3.shell.openExternal(url);
});
require$$1$3.ipcMain.handle("DOWNLOAD_FILE", async (_, { url, filename }) => {
  try {
    if (!mainWin) return { success: false, error: "No main window" };
    const result = await require$$1$3.dialog.showSaveDialog(mainWin, {
    ...
    require$$1.writeFileSync(result.filePath, buffer);
...
require$$1$3.ipcMain.on("TRACKING_STARTED", (_, data) => {
  isTracking = true;
  currentTaskName = data.taskName;
  data.monitoringMode || "TRANSPARENT";
...
require$$1$3.ipcMain.on("TRACKING_STOPPED", () => {
  isTracking = false;
  currentTaskName = "";
  remainingTime = 0;
  activeTasksData = [];
...
require$$1$3.ipcMain.on("TRACKING_TICK", (_, seconds) => {
  if (remainingTime > 0) {
...
require$$1$3.ipcMain.on("TRACKING_TICK_MULTI", (_, data) => {
  activeTasksData.length;
  activeTasksData = data;
  if (data.length > 0) {
    isTracking = true;
    currentTaskName = data[0].taskName + (data[0].subTaskName ? ` - ${data[0].subTaskName}` : "");
    data.reduce((sum, t) => sum + t.elapsedSeconds, 0);
  }
  if (data.length === 0) {
    isTracking = false;
  }
});
require$$1$3.ipcMain.on("FOCUS_MAIN_WINDOW", () => {
  mainWin?.show();
  mainWin?.focus();
});
require$$1$3.ipcMain.on("EXPAND_APP", () => {
  mainWin?.show();
  mainWin?.focus();
});
require$$1$3.ipcMain.on("MINIMIZE_WIDGET", () => {
});
let appUsageBuffer = [];
...
$hwnd = [Win32]::GetForegroundWindow(); $pid = 0; [Win32]::GetWindowThreadProcessId($hwnd, [ref]$pid) | Out-Null; $proc = Get-Process -Id $pid -ErrorAction SilentlyContinue; $title = New-Object System.Text.StringBuilder 256; [Win32]::GetWindowText($hwnd, $title, 256) | Out-Null; Write-Output "$($proc.ProcessName)||$($title.ToString())"`;
        require$$1$4.execFile("powershell", ["-NoProfile", "-Command", psScript], {
          timeout: 3e3,
          windowsHide: true
        }, (err, stdout) => {
          if (err) {
        ...
        require$$1$4.execFile("osascript", ["-e", 'tell application "System Events" to get {name, title of first window} of first application process whose frontmost is true'], {
          timeout: 3e3
        }, (err, stdout) => {
          if (err) {
            resolve(null);
...
function startAppTracking() {
  ...
  appTrackingInterval = setInterval(async () => {
    ...
    if (lastActiveApp && lastActiveApp.appName !== current.appName) {
      const durationSec = Math.round((now - lastActiveApp.startedAt) / 1e3);
      if (durationSec >= 3) {
        appUsageBuffer.push({
          appName: lastActiveApp.appName,
          windowTitle: lastActiveApp.windowTitle,
          startedAt: durationSec
          // Reuse field as durationSec for buffer
        });
...
function stopAppTracking() {
  ...
  if (lastActiveApp) {
    const durationSec = Math.round((Date.now() - lastActiveApp.startedAt) / 1e3);
    if (durationSec >= 3) {
      appUsageBuffer.push({
        appName: lastActiveApp.appName,
        windowTitle: lastActiveApp.windowTitle,
...
require$$1$3.ipcMain.handle("GET_CURRENT_APP", async () => {
  const current = await getActiveWindow();
  return current || { appName: "", windowTitle: "" };
});
require$$1$3.ipcMain.handle("GET_APP_USAGE", async () => {
  ...
  if (lastActiveApp) {
    ...
    lastActiveApp.startedAt = now;
  }
  return entries.map((e) => ({
    appName: e.appName,
    windowTitle: e.windowTitle,
    durationSec: e.startedAt
    // startedAt was repurposed to hold durationSec
  }));
});
...
function setupAutoUpdater() {
  ...
  mainExports.autoUpdater.on("error", (error2) => {
    console.error("Auto-updater error:", error2.message);
    mainWin?.webContents.send("UPDATE_ERROR", error2.message);
  });
  setTimeout(() => {
...
require$$1$3.ipcMain.handle("CHECK_FOR_UPDATES", async () => {
  try {
...
require$$1$3.ipcMain.handle("DOWNLOAD_UPDATE", async () => {
  try {
...
require$$1$3.ipcMain.on("QUIT_AND_INSTALL", () => {
